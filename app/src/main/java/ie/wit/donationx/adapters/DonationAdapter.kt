...
class DonationAdapter constructor(private var donations: List<DonationModel>)
    ...
    inner class MainHolder(val binding : CardDonationBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(donation: DonationModel) {
            binding.valuation.text = ("Valuation: €" + donation.valuation.toDouble().toString())
            binding.guitarMake.text = ("Make: " + donation.guitarMake)
            binding.guitarModel.text = ("Model: " + donation.guitarModel)
            binding.dateView.text = ("Manufactured: " + donation.manufactureDate)
